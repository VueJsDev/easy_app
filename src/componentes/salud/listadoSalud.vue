<template>
	<v-app top-toolbar id="example-9-1" style="min-height: 0;" standalone>
    	...
    	<app-listado :objeto="salud" rutadetalle="detalleSalud"></app-listado>
...
<script>
	...
	export default {
		...
		computed: mapState({
			salud: function(state)
		    {
				let salud = this.$store.state.salud.salud
		        let filtradopor = this.palabra_a_buscar && this.palabra_a_buscar.toLowerCase();
		        if (filtradopor)
		        {
		          salud = salud.filter(function (row)
		          {
