<template>
	<v-app top-toolbar id="example-9-1" style="min-height: 0;" standalone>
    	...
    	<app-listado :objeto="profesionales" rutadetalle="detalleProfesionales"></app-listado>
...
<script>
	...
	export default {
		...
		methods:{
			...
			crear()
			{
				this.$router.push('/profesionales/crear')
			},
		...
		computed: mapState({
			profesionales: function(state)
		    {
				let profesionales = this.$store.state.profesionales.profesionales
		        let filtradopor = this.palabra_a_buscar && this.palabra_a_buscar.toLowerCase();
		        if (filtradopor)
		        {
		          profesionales = profesionales.filter(function (row)
		          {
