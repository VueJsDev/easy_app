<template>
	<v-app top-toolbar id="example-9-1" style="min-height: 0;" standalone>
    	...
    	<app-listado :objeto="bellezas" rutadetalle="detalleBelleza"></app-listado>
...
<script>
	...
	export default {
		...
		methods:{
			...
			crear()
			{
				this.$router.push('/belleza/crear')
			},
		...
		computed: mapState({
			bellezas: function(state)
		    {
				let bellezas = this.$store.state.bellezas.bellezas
		        let filtradopor = this.palabra_a_buscar && this.palabra_a_buscar.toLowerCase();
		        if (filtradopor)
		        {
		          bellezas = bellezas.filter(function (row)
		          {
