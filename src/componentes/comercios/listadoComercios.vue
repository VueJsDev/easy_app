<template>
	<v-app top-toolbar id="example-9-1" style="min-height: 0;" standalone>
    	...
    	<app-listado :objeto="comercios" rutadetalle="detallecomercios"></app-listado>
...
<script>
	...
	export default {
		...
		methods:{
			...
			crear()
			{
				this.$router.push('/comercios/crear')
			},
		...
		computed: mapState({
			comercios: function(state)
			{
				let comercios = this.$store.state.comercios.comercios
				let filtradopor = this.palabra_a_buscar && this.palabra_a_buscar.toLowerCase();
					if (filtradopor)
					{
						comercios = comercios.filter(function (row)
						{
